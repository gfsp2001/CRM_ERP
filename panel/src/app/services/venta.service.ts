...
export class VentaService {
  ...
  enviar_ventas_hoy(token: any): Observable<any> {
    let headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this._http.get(this.url + 'enviar_ventas_hoy', { headers: headers });
  }

  enviar_ventas_fechas(inicio: any, hasta: any, asesor: any, token: any): Observable<any> {
    let headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this._http.get(this.url + 'enviar_ventas_fechas/' + inicio + '/' + hasta + '/' + asesor, { headers: headers });
  }
