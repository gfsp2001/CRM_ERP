...
const generar_matricula_admin = async function (req, res) {
    ...
    if (req.user) {
        ...
        for (var item of data.detalles) {
            ...
            aumentar_aforo_admin(item.ciclo_salon);
        }
...
const aumentar_aforo_admin = async function (id) {
    let salon = await Ciclo_salon.findById({ _id: id });
    let nuevo_aforo_actual = salon.aforo_actual + 1;
    await Ciclo_salon.findByIdAndUpdate({ _id: id }, { aforo_actual: nuevo_aforo_actual });
}
...
const send_invoice = async function (req, res) {
    if (req.user) {
        ...
        generar_actividad_matricula(id, 'Se reenvio la orden al cliente.');
...
const enviar_orden_matricula = async function (id) {
    ...
    readHTMLFile(process.cwd() + '/mails/invoice_matricula.html', (err, html) => {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
                console.log('Email didnt send: ' + error.response);
            } else {
...
const enviar_matriculas_hoy = async function (req, res) {
    ...
    if (req.user) {
        ...
        let matriculas = await Matricula.find({ dia: dia, mes: mes, year: year }).populate('cliente').populate('asesor').sort({ createdAt: 1 });
...
const enviar_matriculas_fechas = async function (req, res) {
...
const obtener_matriculas_admin = async function (req, res) {
...
const cancelar_matricula_admin = async function (req, res) {
    ...
    if (req.user) {
        ...
        if (req.user.rol == 'Administrador') {
            ...
            let id = req.params.id;
            let matricula = await Matricula.findByIdAndUpdate({ _id: id }, { estado: 'Cancelado' });
            let detalles = await Matricula_detalle.find({ matricula: id });
            for (var item of detalles) {
                cancelar_aforo_admin(item.ciclo_salon);
            }
            await Matricula_detalle.updateMany({ matricula: id }, { estado: 'Cancelado' })
            await Pago.updateMany({ matricula: id }, { estado: 'Cancelado' })
            ...
            res.status(200).send({ data: undefined, message: 'No tienes permisos para esta acción.' });
        }
...
module.exports = {
    ...
    send_invoice,
    enviar_matriculas_hoy,
    enviar_matriculas_fechas,
    obtener_matriculas_admin,
